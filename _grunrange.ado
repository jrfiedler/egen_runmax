...
program define _grunrange
	...
	mata: input_lengths("`varlist'", "`vn'", "`touse'")
end

mata
	void input_lengths(string scalar input_name,
	                   string scalar output_name,
                       string scalar touse_name)
	{
		real scalar N, group_num, n_groups, min_i, max_i, len_i
		real colvector group_mins, group_maxes, seen
		...
		for (i = 1; i <= N; i++) {
			...
			if (!seen[group_num]) {
				...
				max_i = max((group_maxes[group_num], input[i]))
				len_i = max_i - min_i
			}
			output[i] = len_i
			group_mins[group_num] = min_i
			group_maxes[group_num] = max_i
		}
	}
